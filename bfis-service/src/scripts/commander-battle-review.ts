...
import { decodeWeapons, type DecodedWeapon } from "../snapshot/weapon-decoder.js";
...
async function main(): Promise<void> {
  try {
    ...
    const unitAnalysis = {
      ...
      byCoalition: {} as Record<string, number>,
      byType: {} as Record<string, number>,
      byCategory: {} as Record<string, number>,
      withPositions: 0,
      airAssets: [] as Array<{ id: string; type: string; name?: string; coalition: string; position: { lat: number; lon: number; altMeters: number } }>,
      groundAssets: [] as Array<{ id: string; type: string; name?: string; coalition: string; position: { lat: number; lon: number; altMeters: number } }>,
      navalAssets: [] as Array<{ id: string; type: string; name?: string; coalition: string; position: { lat: number; lon: number; altMeters: number } }>,
      ghostUnits: [] as Array<{ id: string; type: string; name?: string; coalition: string; category: string; classification: 'player-slot' | 'dead-unit' }>,
    };
    ...
    snapshot.base.units.forEach(unit => {
      ...
      if (coalition === 'UNKNOWN' && isDefaultPosition) {
        ...
        const classification: 'player-slot' | 'dead-unit' = 
          (category && category !== 'Unknown') ? 'dead-unit' : 'player-slot';
    ...
    const groupsMap = new Map<string, Array<{ id: string; type: string; name?: string; coalition: string; category: string }>>();
    const unitsWithoutGroup: Array<{ id: string; type: string; name?: string; coalition: string; category: string }> = [];
    ...
    // Group by coalition
    const groupsByCoalition: Record<string, Array<{ groupId: string; units: Array<{ id: string; type: string; name?: string; coalition: string; category: string }> }>> = {};
    ...
    if (unitsWithoutGroup.length > 0) {
      console.log(`Units without Group (${unitsWithoutGroup.length}):`);
      const ungroupedByCoalition: Record<string, number> = {};
    ...
    // Airbases
    const airbaseAnalysis = { total: snapshot.airbases.length, byCoalition: {} as Record<string, number> };
    ...
    const weaponCache = (reader as any).weaponCache as Map<number, DecodedWeapon>;
    ...
    if (activeWeapons.length > 0) {
      ...
      const weaponAnalysis = {
        ...
        totalSpeed: 0,
        totalAltitude: 0,
        fastest: null as DecodedWeapon | null,
        highest: null as DecodedWeapon | null,
      };
      ...
      activeWeapons.forEach(weapon => {
        ...
        if (speed < 100) weaponAnalysis.bySpeed.slow++;
        else if (speed < 500) weaponAnalysis.bySpeed.medium++;
        else if (speed < 1000) weaponAnalysis.bySpeed.fast++;
        else weaponAnalysis.bySpeed.veryFast++;
        
        // Altitude categorization
        if (alt < 1000) weaponAnalysis.byAltitude.low++;
        else if (alt < 5000) weaponAnalysis.byAltitude.medium++;
        else if (alt < 15000) weaponAnalysis.byAltitude.high++;
        else weaponAnalysis.byAltitude.veryHigh++;
    ...
    if (neutralUnits > 0) console.log(`  NEUTRAL Forces: ${neutralUnits} units`);
    ...
    if (blueAirbases > 0) console.log(`  BLUE Controlled: ${blueAirbases} airbases`);
    if (redAirbases > 0) console.log(`  RED Controlled: ${redAirbases} airbases`);
    if (neutralAirbases > 0) console.log(`  Neutral Airbases: ${neutralAirbases} (available for either side)`);
    console.log('');
