...
/**
 * Interface for NDJSON logger instances.
 *
 * Provides a simple log method that accepts any JSON-serializable object
 * and appends it as a single line to the NDJSON file.
 */
export interface NdjsonLogger {
  /**
   * Append a JSON record to the NDJSON log file.
   *
   * The record is serialized to JSON and written as a single line.
   * Each call results in a separate write operation for durability.
   *
   * @param record - Any JSON-serializable object to log
   */
