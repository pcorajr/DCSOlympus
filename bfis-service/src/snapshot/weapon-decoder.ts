...
/**
 * Decoded weapon data structure.
 * 
 * Mirrors the weapon data structure from the Olympus frontend.
 */
export interface DecodedWeapon {
  /** Weapon ID (uint32 from buffer) */
  weaponId: number;
  /** Weapon category (e.g., "Missile", "Bomb", "Shell") */
  category: string;
  /** Whether the weapon is alive/active */
  alive: boolean;
  /** Weapon coalition */
  coalition: OlympusCoalition;
  /** Weapon type name */
  name: string;
  /** Weapon position (lat, lon, altMeters) */
  position: BfisLatLng;
  /** Weapon speed in m/s */
  speed: number;
  /** Weapon heading in radians */
  heading: number;
  /** Update time for this weapon (from buffer updateTime) */
  updateTime: number;
}
 ...
 * ```
 */
export function decodeWeapons(buffer: ArrayBuffer, weaponCache: Map<number, DecodedWeapon>): { updateTime: number; weapons: DecodedWeapon[] } {
