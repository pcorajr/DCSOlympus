 ...
 */
function resolveOlympusBaseUrls() {
 ...
 */
function resolveOlympusAuth() {
    const roleEnv = (process.env.BFIS_OLYMPUS_ROLE ?? "GAME_MASTER").toUpperCase();
    let role = "GAME_MASTER";
    if (roleEnv === "BLUE_COMMANDER")
        role = "BLUE_COMMANDER";
    else if (roleEnv === "RED_COMMANDER")
        role = "RED_COMMANDER";
    else if (roleEnv === "ADMIN")
        role = "ADMIN";
    ...
    if (!username || !password) {
        throw new Error("Missing Olympus credentials. Ensure /home/dcs/.creds/olympus_env.txt exists or BFIS_OLYMPUS_USERNAME/BFIS_OLYMPUS_PASSWORD or OLYMPUS_* vars are set.");
    }
    return {
        role,
        username,
