 ...
 */
function createMockFetchForAllEndpoints(missionData, unitsBuffer, weaponsBuffer) {
    return async (url) => {
    ...
    describe("User Story 3: Mission Resets and State Changes", () => {
        ...
        test("T035: lastTimes reset to empty on session hash change (unit test)", { skip: RUN_INTEGRATION_TESTS }, async () => {
            ...
            try {
                ...
                const lastTimes = reader.lastTimes;
        ...
        test("T036: session hash change mid-poll cycle aborts immediately (unit test)", { skip: RUN_INTEGRATION_TESTS }, async () => {
            ...
            try {
                ...
                await reader.readOnce();
                assert.strictEqual(reader.lastSessionHash, "hash1");
                ...
                assert.strictEqual(reader.lastSessionHash, "hash2");
    ...
    describe("User Story 4: Polling Multiple Data Sources", () => {
        test("T043: readOnce fetches all endpoints in specified order (unit test)", { skip: RUN_INTEGRATION_TESTS }, async () => {
            ...
            globalThis.fetch = async (url) => {
                ...
                // Track fetch order
                if (urlStr.includes("/mission"))
                    fetchOrder.push("mission");
                else if (urlStr.includes("/units"))
                    fetchOrder.push("units");
                else if (urlStr.includes("/weapons"))
                    fetchOrder.push("weapons");
                else if (urlStr.includes("/logs"))
                    fetchOrder.push("logs");
                else if (urlStr.includes("/airbases"))
                    fetchOrder.push("airbases");
                else if (urlStr.includes("/bullseyes"))
                    fetchOrder.push("bullseyes");
                else if (urlStr.includes("/spots"))
                    fetchOrder.push("spots");
                else if (urlStr.includes("/drawings"))
                    fetchOrder.push("drawings");
        ...
        test("T070: incremental update size reduction (SC-006) comparing full vs incremental fetch sizes (unit test)", { skip: RUN_INTEGRATION_TESTS }, async () => {
            ...
            try {
                ...
                if (fullRefresh && incremental) {
                    ...
                    assert.ok(incremental.bytes < fullRefresh.bytes, `Incremental update (${incremental.bytes} bytes) should be smaller than full refresh (${fullRefresh.bytes} bytes)`);
