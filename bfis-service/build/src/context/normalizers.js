 ...
 */
export function normalizeAirbases(raw, logger) {
    if (!raw || typeof raw !== "object")
        return [];
    const container = raw;
    if (!container.airbases || typeof container.airbases !== "object")
        return [];
    const result = [];
    const airbasesObj = container.airbases;
 ...
 */
export function normalizeBullseyes(raw) {
    if (!raw || typeof raw !== "object")
        return [];
    ...
    for (const [key, val] of Object.entries(rawObj)) {
        if (key === "time" || key === "sessionHash" || typeof val !== "object")
            continue;
        // Map numeric keys to coalitions
        let coalition = "UNKNOWN";
        if (key === "1")
            coalition = "RED";
        if (key === "2")
            coalition = "BLUE";
 ...
 */
export function normalizeSpots(raw, logger) {
    if (!raw || typeof raw !== "object")
        return [];
    const container = raw;
    if (!container.spots || typeof container.spots !== "object")
        return [];
    const result = [];
    const spotsObj = container.spots;
    // Handle both array and object formats
 ...
 */
export function normalizeDrawings(raw, logger) {
    if (!raw || typeof raw !== "object")
        return [];
    const container = raw;
    if (!container.drawings || typeof container.drawings !== "object")
        return [];
    const result = [];
    const drawingsObj = container.drawings;
    ...
    function extractDrawingsFromValue(value, path = "") {
...
export function normalizeLogs(raw) {
    if (!raw || typeof raw !== "object")
        return [];
    const container = raw;
    if (!container.logs || typeof container.logs !== "object")
        return [];
    const result = [];
    const logsObj = container.logs;
    // Handle both array and object formats
 ...
 */
export function buildWeaponsSummary(decodedWeapons) {
